<template>
  <FluidContainer class="flex flex-col">
    <div class="flex flex-row flex-wrap md:grid md:grid-cols-3 border-b pb-4 mb-4 dark:border-dark-200">
      <h1 class="text-xl text-gray-500 dark:text-dark-fg">{{ repoOwner }}</h1>
    ...
    <div class="space-y-4">
      ...
      >
        <span class="text-gray-500 dark:text-dark-fg">{{ `${repo.name}` }}</span>
      </ListItem>
    </div>
    <div v-if="(searchedRepos || []).length <= 0" class="text-center">
      <span class="text-gray-500 dark:text-dark-fg m-auto">This organization / user does not have any projects yet.</span>
    </div>
  </FluidContainer>
</template>
