    ...
    <div class="flex w-full min-h-0 flex-grow">
      ...
      <div class="flex flex-grow relative">
        <div v-if="build.error" class="flex flex-col p-4">
          <span class="text-red-400 dark:text-orange-300 font-bold text-xl mb-2">Execution error</span>
          <span class="text-red-400 dark:text-orange-300">{{ build.error }}</span>
        </div>

        <div v-else-if="build.status === 'blocked'" class="flex flex-col flex-grow justify-center items-center">
          <Icon name="status-blocked" class="w-32 h-32 text-gray-500 dark:text-dark-fg" />
          <p class="text-xl text-gray-500 dark:text-dark-fg">This pipeline is awaiting approval by some maintainer!</p>
        ...
        <div v-else-if="build.status === 'declined'" class="flex flex-col flex-grow justify-center items-center">
          <Icon name="status-blocked" class="w-32 h-32 text-gray-500 dark:text-dark-fg" />
          <p class="text-xl text-gray-500 dark:text-dark-fg">This pipeline has been declined!</p>
        </div>
